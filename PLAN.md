...
12. Hidden salary must not leak through JSON-LD, APIs, HTML payloads, feeds, social messages or per-job comparison percentages. Exclude the per-job percentage box for hidden salaries; include their ranges only in aggregates meeting the minimum cohort size.
...
Phase 1 work is local: shared pricing/flags/taxonomies, canonical reference datasets, additive migrations and posting validation. No new migration or feature flag has been applied online. Stripe configuration is unchanged. See docs/qa for verification evidence as each phase is completed.
