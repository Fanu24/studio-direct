import {formatUtcOffset,resolveJobLocations,type JobAddons} from '@gaming/shared';
...
export async function ownedNativeListing(db:Database,userId:string,id:string){
  const row=await db.prepare(`SELECT d.input_json,d.addons_json,j.tenant_id,j.slug,o.status FROM native_listing_details d
    ...
    WHERE j.id=? AND l.user_id=?`).bind(id,userId).first<{input_json:string;addons_json:string;tenant_id:string;slug:string;status:string}>();
  if(!row)return null;
...
export async function editNativeListing(db:Database,userId:string,id:string,raw:unknown,now=new Date()){
  ...
  const location=await nativeLocation(db,input),guard=`EXISTS(SELECT 1 FROM employer_listings l JOIN employer_orders o ON o.id=l.order_id WHERE l.job_id=? AND l.user_id=? AND o.status='paid')`,values=[id,userId];
  await db.batch([
