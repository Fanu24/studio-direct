...
export async function ownedListing(db:Database,userId:string,id:string){
 ...
 applyUrl:row.apply_mode==='external'?row.apply_url:'',contactEmail:row.contact_email,tags:tags.results.map(t=>t.tag_slug),primarySkill:saved.primarySkill||tags.results[0]?.tag_slug,
...
export async function editListing(db:Database,userId:string,id:string,input:ListingInput,now=new Date()){
 ...
 ...input.tags.flatMap(tag=>[db.prepare('INSERT OR IGNORE INTO tags(slug,label) VALUES(?,?)').bind(tag,tag.replaceAll('-',' ')),db.prepare(`INSERT OR IGNORE INTO job_tags(job_id,tag_slug) SELECT ?,? WHERE ${guard}`).bind(id,tag,id,userId)]),
