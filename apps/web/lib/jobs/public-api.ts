import {SKILL_SEEDS,isJobTag,isCountrySlug,isCitySlug,isRegionSlug} from '@gaming/shared';
...
export function apiFilters(params:URLSearchParams){
 ...
 if(tag&&!isJobTag(tag)&&!SKILL_SEEDS.some(skill=>skill.slug===tag))throw Error('Unknown tag');if(location&&!isCountrySlug(location)&&!isCitySlug(location)&&!isRegionSlug(location))throw Error('Unknown location');
 const filters:JobListFilters={tag,locationSlug:location,remoteOnly:boolean('remote'),cryptoPayment:boolean('crypto_payment'),page:number('page',1,10000,1),pageSize:number(params.has('limit')?'limit':'page_size',1,100,20),salaryMin:number('salary_min',0,10000000),salaryMax:number('salary_max',0,10000000)};
...
export async function publicJobsApi(request:Request,format:'json'|'rss'='json',providedEnv?:PlatformEnv){
 ...
 const jobs=result.jobs.map((job,index)=>({id:job.id,slug:job.slug,externalId:job.externalId,title:job.title,companyName:job.companyName,companySlug:job.companySlug,companyLogoUrl:job.companyLogoUrl?new URL(job.companyLogoUrl,origin).href:null,location:job.location,remote:job.remote,salaryText:job.salaryText,salaryMin:job.salaryMin,salaryMax:job.salaryMax,salaryCurrency:job.salaryCurrency??null,salaryPeriod:job.salaryPeriod??null,cryptoPaymentAvailable:job.cryptoPaymentAvailable===1,postedAt:job.postedAt,tags:job.tags,apply_url:new URL(jobPublicHref(job),origin).href,...(query.description?{description:details[index]?.descriptionHtml??''}:{})}));
