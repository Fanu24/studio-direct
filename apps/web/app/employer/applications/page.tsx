...
export default async function Applications({searchParams}:{searchParams:Promise<{status?:string;job?:string;page?:string}>}){
 const env=await platform(),user=await currentUser(env);if(!user)redirect('/employer/login?next=/employer/applications');
 const f=await searchParams,page=Math.min(10000,Math.max(1,Math.floor(Number(f.page)||1)));const conditions=['l.user_id=?',"a.status!='withdrawn'"],values:unknown[]=[user.id];
