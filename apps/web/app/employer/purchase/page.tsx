...
export default async function Purchase({searchParams}:{searchParams:Promise<{order?:string}>}){
  ...
  return <main className="container stack"><h1>Your job purchase</h1>
    {order.status==='pending'?<><CheckoutStatus orderId={id}/><NativePaymentCheck orderId={id}/><p>Your company account activates after payment confirmation.</p></>:<p>Order status: {order.status}.</p>}
