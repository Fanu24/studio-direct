...
describe("EntryDeveloperJobsPage", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("intersects dev and entry-level, distinct from intern jobs", async () => {
    const { default: EntryDeveloperJobsPage } = await import("./page");
    const page = await EntryDeveloperJobsPage({
    ...
    expect(copy).toContain("dev and entry-level");
    expect(copy).not.toMatch(/[–—]/);
    expect(hrefs).toContain("/entry-level-jobs");
