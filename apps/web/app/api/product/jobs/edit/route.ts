...
export async function POST(request:Request){
  ...
  try{const raw=await request.json();if(typeof raw.jobId!=='string')throw Error('Invalid listing.');const slug=await editNativeListing(env.DB,user.id,raw.jobId,raw.listing);const privateJob=await env.DB.prepare('SELECT confidential FROM jobs WHERE id=?').bind(raw.jobId).first<number>('confidential');return Response.json({url:privateJob?'/private-jobs/'+encodeURIComponent(raw.jobId):'/jobs/'+slug});}
