...
export default async function Notifications(){const env=await platform(),user=await currentUser(env);if(!user)redirect('/login?next=/notifications');
 const rows=await env.DB.prepare('SELECT id,subject,body,destination_path,read_at,created_at FROM notification_outbox n WHERE user_id=? AND NOT EXISTS(SELECT 1 FROM notification_preferences p WHERE p.user_id=n.user_id AND p.kind=n.kind AND p.in_app_enabled=0) ORDER BY created_at DESC LIMIT 100').bind(user.id).all<{id:string;subject:string;body:string;destination_path:string;read_at:string|null;created_at:string}>();
